...
from spectree.plugins.lilya_plugin import PydanticResponse
...
api = SpecTree("lilya", before=before_handler, after=after_handler, annotations=True)
        ...
        Include(
            ...
            routes=[
                Include(
                    "/user",
                    routes=[
                        Path("/{name}", user_score, methods=["POST"]),
                    ],
                ),
                Include(
                    "/user_annotated",
                    routes=[
                        Path("/{name}", user_score_annotated, methods=["POST"]),
                    ],
                ),
                Include(
                    "/user_skip",
                    routes=[
                        Path("/{name}", user_score_skip, methods=["POST"]),
                    ],
                ),
                Include(
                    "/user_model",
                    routes=[
                        Path("/{name}", user_score_model, methods=["POST"]),
                    ],
                ),
                Path("/no_response", no_response, methods=["POST", "GET"]),
                Path("/file_upload", file_upload, methods=["POST"]),
                Path("/list_json", list_json, methods=["POST"]),
                Path("/return_list", return_list, methods=["GET"]),
                Path("/return_root", return_root, methods=["GET"]),
                Path("/return_optional_alias", return_optional_alias, methods=["GET"]),
                Path("/custom_error", custom_error, methods=["POST"]),
            ],
        ),
        Include("/static", app=StaticFiles(directory="docs"), name="static"),
    ]
)
...
def inner_register_func():
    ...
    app.routes.append(Path("/api/user/{name}/address/{address_id}", user_address))
...
def test_lilya_validate(client):
    ...
    for fragment in ("user", "user_annotated"):
        resp = client.post(f"/api/{fragment}/lilya")
        assert resp.status_code == 422
        assert resp.headers.get("X-Error") == "Validation Error"

        client.cookies = dict(pub="abcdefg")
        resp = client.post(
            f"/api/{fragment}/lilya?order=1",
            json=dict(name="lilya", limit=10),
        )
        resp_body = resp.json()
        assert resp_body["name"] == "lilya"
        assert resp_body["score"] == sorted(resp_body["score"], reverse=True)
        assert resp.headers.get("X-Validation") == "Pass"

        resp = client.post(
            f"/api/{fragment}/lilya?order=0",
            json=dict(name="lilya", limit=10),
        )
        resp_body = resp.json()
        assert resp_body["name"] == "lilya"
...
def test_lilya_skip_validation(client, response_format: str):
    client.cookies = dict(pub="abcdefg")
    assert response_format in ("json", "xml")
    resp = client.post(
        f"/api/user_skip/lilya?order=1&response_format={response_format}",
        json=dict(name="lilya", limit=10),
    )
    assert resp.headers.get("X-Validation") == "Pass"
    if response_format == "json":
        ...
        assert user_xml_data.name == "lilya"
        assert user_xml_data.score == sorted(user_xml_data.score, reverse=True)


def test_lilya_return_model(client):
    client.cookies = dict(pub="abcdefg")
    resp = client.post(
        "/api/user_model/lilya?order=1",
        json=dict(name="lilya", limit=10),
    )
    resp_body = resp.json()
    assert resp_body["name"] == "lilya"
...
def test_client_and_api(request):
    ...
    app = Lilya(routes=[Path("/ping", Ping)])
    api.register(app)
...
def test_lilya_doc(test_client_and_api, expected_doc_pages):
...
def test_lilya_no_response(client):
    resp = client.get("/api/no_response")
    assert resp.status_code == 200, resp.text

    resp = client.post("/api/no_response", json={"name": "lilya", "limit": "1"})
    assert resp.status_code == 200, resp.text


def test_json_list_request(client):
    resp = client.post("/api/list_json", json=[{"name": "lilya", "limit": "1"}])
    assert resp.status_code == 200, resp.text


@pytest.mark.parametrize("pre_serialize", [False, True])
def test_lilya_return_list_request(client, pre_serialize: bool):
...
def test_lilya_return_root_request_sync(client, return_what: str):
...
def test_lilya_upload_file(client):
    file_content = "abcdef"
...
def test_lilya_return_optional_alias(client):
