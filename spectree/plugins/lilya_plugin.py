...
from spectree.utils import cached_type_hints, get_multidict_items_lilya
...
class LilyaPlugin(BasePlugin):
    ASYNC = True

    def __init__(self, spectree):
        super().__init__(spectree)
        self.conv2type = {conv: typ for typ, conv in TRANSFORMER_TYPES.items()}
    ...
    async def request_validation(self, request, query, json, form, headers, cookies):
        ...
        request.context = Context(
            query.parse_obj(get_multidict_items_lilya(request.query_params))
            if query
            else None,
    ...
    async def validate(
        ...
        **kwargs: Any,
        ...
        if "request" in kwargs:
            instance, request = None, kwargs.get("request")
    ...
    def find_routes(self):
        ...
        def parse_route(app, prefix=""):
            # :class:`lilya.staticfiles.StaticFiles` doesn't have routes
            if not app.routes:
                return
    ...
    def parse_path(self, route, path_parameter_descriptions):
        _, path, variables, _ = compile_path(route.path)
        parameters = []
